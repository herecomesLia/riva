...
type WorkPickerType = 'endPeriod' | 'startPeriod'
...
const WORK_END_YEAR_OPTIONS = ['至今', ...PERIOD_YEARS]
...
function getWorkDraftLabel(draft: WorkDraft, index: number) {
  return draft.organization || draft.title || `工作经历 ${index + 1}`
}

function getWorkSummary(sourceText: string, fallback: string) {
  const normalizedText = sourceText.trim()
  const lines = normalizedText
    .split(/\n+/)
    .map((line) => line.trim())
...
function createWorkDraft(item: ResumeExperience): WorkDraft {
  const { startPeriod, endPeriod } = parseEducationPeriod(item.period)
...
function workDraftToExperience(draft: WorkDraft, originalItem?: ResumeExperience): ResumeExperience {
  const sourceText = draft.sourceText.trim()

  return {
    description: getWorkSummary(sourceText, originalItem?.description || ''),
    id: draft.id,
    organization: draft.organization.trim(),
    period: `${draft.startPeriod} - ${draft.endPeriod}`,
    sourceText,
...
function ExperienceList({ emptyText = '暂无经历信息', items }: { emptyText?: string; items: ResumeExperience[] }) {
  if (items.length === 0) {
      ...
      {items.map((item) => (
        <article key={item.id}>
          <div className="resume-experience-card__topbar">
            <h3>{item.title}</h3>
            <span className="resume-experience-card__period">{item.period}</span>
          </div>
...
export function ResumeProfilePage({ onSetupResume }: ResumeProfilePageProps) {
  ...
  const [isEditingWorkExperiences, setIsEditingWorkExperiences] = useState(false)
  const [workDrafts, setWorkDrafts] = useState<WorkDraft[]>([])
  const [activeWorkDraftId, setActiveWorkDraftId] = useState('')
  const [workPickerType, setWorkPickerType] = useState<WorkPickerType | ''>('')
  const [workPickerYear, setWorkPickerYear] = useState('')
  const [workPickerMonth, setWorkPickerMonth] = useState('')
  ...
  function openWorkEditor() {
    const drafts = workExperiences.length ? workExperiences.map(createWorkDraft) : [createEmptyWorkDraft()]

    setWorkDrafts(drafts)
    setActiveWorkDraftId(drafts[0]?.id || '')
    setIsEditingWorkExperiences(true)
  }

  function closeWorkEditor() {
    setIsEditingWorkExperiences(false)
    setWorkDrafts([])
    setActiveWorkDraftId('')
    setWorkPickerType('')
  }
  ...
  function handleWorkSave(event: FormEvent<HTMLFormElement>) {
    ...
    setWorkExperiences((currentItems) =>
      workDrafts.map((draft) =>
        workDraftToExperience(
          draft,
          currentItems.find((item) => item.id === draft.id),
        ),
      ),
    )
    closeWorkEditor()
  ...
  function updateWorkDraft(id: string, patch: Partial<WorkDraft>) {
    setWorkDrafts((currentDrafts) =>
      currentDrafts.map((draft) => (draft.id === id ? { ...draft, ...patch } : draft)),
    )
  }

  function addWorkDraft() {
    const draft = createEmptyWorkDraft()

    setWorkDrafts((currentDrafts) => [...currentDrafts, draft])
    setActiveWorkDraftId(draft.id)
  }

  function deleteWorkDraft(id: string) {
    setWorkDrafts((currentDrafts) => {
      const nextDrafts =
        currentDrafts.length > 1 ? currentDrafts.filter((draft) => draft.id !== id) : [createEmptyWorkDraft()]

      if (!nextDrafts.some((draft) => draft.id === activeWorkDraftId)) {
        setActiveWorkDraftId(nextDrafts[0]?.id || '')
      }
  ...
  function openWorkPicker(type: WorkPickerType) {
    const activeDraft = workDrafts.find((draft) => draft.id === activeWorkDraftId)

    if (!activeDraft) {
      return
    }

    const period = type === 'startPeriod' ? activeDraft.startPeriod : activeDraft.endPeriod
    const { month, year } = splitPeriodValue(period === '至今' ? '' : period)

    setWorkPickerType(type)
    setWorkPickerYear(period === '至今' ? '至今' : year || String(CURRENT_YEAR))
    setWorkPickerMonth(month || '01')
  }
  ...
  function confirmWorkPicker() {
    ...
    if (!activeDraft || !workPickerType) {
      return
    }

    updateWorkDraft(activeDraft.id, {
      [workPickerType]:
        workPickerType === 'endPeriod' && workPickerYear === '至今'
          ? '至今'
          : mergePeriodValue(workPickerYear, workPickerMonth),
    })
    closeWorkPicker()
  }
  ...
  const activeWorkDraft = workDrafts.find((draft) => draft.id === activeWorkDraftId)
  const workPickerTitle = workPickerType === 'startPeriod' ? '入职时间' : '离职时间'
    ...
    <div className="resume-page" aria-labelledby="resume-profile-title">
      ...
      <div className="resume-profile-grid resume-profile-grid--wide resume-experience-grid">
        ...
        <section className="panel resume-balanced-scroll-panel">
          ...
          <ExperienceList emptyText="暂无内容" items={workExperiences} />
        </section>
      </div>
      ...
      {isEditingWorkExperiences && activeWorkDraft ? (
        <div className="resume-modal-backdrop">
          <form
          ...
          >
            <div className="resume-project-editor__header">
              <div>
                <p id="work-editor-title">编辑工作经历</p>
                <span>维护结构化信息，工作内容概述由 Riva 根据原文生成</span>
              </div>
            </div>

            <div className="resume-education-editor__body">
              <div className="resume-education-editor__nav" aria-label="选择工作经历">
                {workDrafts.map((draft, index) => (
                  <button
                    aria-pressed={draft.id === activeWorkDraft.id}
                    className={`resume-education-editor__nav-item${
                      draft.id === activeWorkDraft.id ? ' resume-education-editor__nav-item--active' : ''
                    }`}
                    key={draft.id}
                    type="button"
                    onClick={() => setActiveWorkDraftId(draft.id)}
                  >
                    {getWorkDraftLabel(draft, index)}
                  </button>
                ))}
                <button
                  aria-label="新增工作经历"
              ...
              <div className="resume-education-editor__form" onFocusCapture={handleStructuredEditorTextFocus}>
                <label className="form-field">
                  ...
                  <input
                    placeholder="请填写公司名称"
                    value={activeWorkDraft.organization}
                    onChange={(event) => updateWorkDraft(activeWorkDraft.id, { organization: event.target.value })}
                  />
                </label>
                ...
                <label className="form-field">
                  ...
                  <input
                    placeholder="请填写职位名称"
                    value={activeWorkDraft.title}
                    onChange={(event) => updateWorkDraft(activeWorkDraft.id, { title: event.target.value })}
                  />
                </label>
                ...
                <div className="resume-education-editor__periods">
                  <div className="form-field">
                    ...
                    <button
                      className={`resume-education-editor__field-button${
                        activeWorkDraft.startPeriod ? '' : ' resume-education-editor__field-button--empty'
                      }`}
                      type="button"
                      onClick={() => openWorkPicker('startPeriod')}
                    >
                      {activeWorkDraft.startPeriod || '请选择'}
                    </button>
                  </div>
                  ...
                  <div className="form-field">
                    ...
                    <button
                      className={`resume-education-editor__field-button${
                        activeWorkDraft.endPeriod ? '' : ' resume-education-editor__field-button--empty'
                      }`}
                      type="button"
                      onClick={() => openWorkPicker('endPeriod')}
                    >
                      {activeWorkDraft.endPeriod || '请选择'}
                    </button>
                  </div>
                </div>
                ...
                <label className="form-field">
                  ...
                  <textarea
                    className="resume-project-editor__textarea resume-project-editor__textarea--compact"
                    placeholder="请填写工作内容描述"
                    value={activeWorkDraft.sourceText}
                    onChange={(event) => updateWorkDraft(activeWorkDraft.id, { sourceText: event.target.value })}
                  />
                </label>
                ...
                <button
                  className="resume-education-editor__delete"
                  type="button"
                  onClick={() => deleteWorkDraft(activeWorkDraft.id)}
                >
                  删除这段工作经历
                </button>
              </div>
          ...
          {workPickerType ? (
            <div className="education-picker-sheet" role="dialog" aria-label={`选择${workPickerTitle}`}>
              <div className="education-picker-sheet__header">
                <button type="button" onClick={closeWorkPicker}>
                  取消
                </button>
                <strong>{workPickerTitle}</strong>
                <button type="button" onClick={confirmWorkPicker}>
                  确定
                </button>
              </div>
              <div className={workPickerType === 'endPeriod' && workPickerYear === '至今' ? '' : 'wheel-picker-group'}>
                <WheelPicker
                  label={`选择${workPickerTitle}年份`}
                  options={workPickerType === 'endPeriod' ? WORK_END_YEAR_OPTIONS : PERIOD_YEARS}
                  value={workPickerYear}
                  onChange={setWorkPickerYear}
                />
                {workPickerType === 'endPeriod' && workPickerYear === '至今' ? null : (
                  <WheelPicker
                    label={`选择${workPickerTitle}月份`}
                    options={PERIOD_MONTHS}
                    value={workPickerMonth}
                    onChange={setWorkPickerMonth}
                  />
                )}
              </div>
            </div>
