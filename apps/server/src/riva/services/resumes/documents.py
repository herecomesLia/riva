...
from riva.integrations.storage import (
    ResumeObjectStorage,
    ResumeStorageError,
    StoredResumeObject,
...
class ResumeDocumentService:
    ...
    async def create_file_document(
        ...
        *,
        ...
        try:
            ...
            raise InvalidDataError(RESUME_FILENAME_INVALID) from None
        ...
        try:
            ...
            raise _storage_service_error(error.code) from None
        except Exception:
            raise ExternalDependencyError("resume_storage_unavailable") from None
        ...
        try:
            ...
            try:
                ...
                await self._delete_safely(stored.key)
                raise ExternalDependencyError(RESUME_UPLOAD_UNAVAILABLE) from None
    ...
    async def create_pasted_text_document(
        ...
        *,
        ...
        if not isinstance(text, str) or not text.strip():
            raise InvalidDataError(RESUME_TEXT_EMPTY)
        try:
            raw_text = text.encode("utf-8")
        except UnicodeError:
            raise InvalidDataError(RESUME_TEXT_INVALID) from None
        if len(raw_text) > self.max_upload_bytes:
            raise DataTooLargeError(RESUME_TEXT_TOO_LARGE)
        ...
        try:
            ...
            raise _pasted_text_service_error(error.code) from None
    ...
    async def get_document(
        ...
        if document is None:
            raise ResourceMissingError("resume_document_not_found")
...
def _storage_service_error(code: str) -> ServiceError:
    if code == "resume_file_empty":
        return InvalidDataError("resume_file_empty")
    if code == "resume_file_too_large":
        return DataTooLargeError("resume_file_too_large")
    return ExternalDependencyError("resume_storage_unavailable")


def _pasted_text_service_error(code: str) -> ServiceError:
    if code == RESUME_TEXT_EMPTY:
        return InvalidDataError(RESUME_TEXT_EMPTY)
    if code == RESUME_TEXT_TOO_LARGE:
        return DataTooLargeError(RESUME_TEXT_TOO_LARGE)
    return InvalidDataError(RESUME_TEXT_INVALID)
