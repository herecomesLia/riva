...
from riva.agents.jobs.jd_parser_types import AnalysisItemList
...
class _ResumeImportDomainModel(DomainModel):
    model_config = ConfigDict(extra="forbid")


class ResumeImportSkippedItemResponse(_ResumeImportDomainModel):
    section: ResumeImportSection
    source_index: int = Field(ge=0)
    reasons: list[ResumeImportSkipReason] = Field(min_length=1)


class ResumeImportProtectedItemResponse(_ResumeImportDomainModel):
    section: ResumeImportSection
    item_id: StandardUUID
    source: ResumeImportProtectedSource


class ResumeImportChangeSummaryResponse(_ResumeImportDomainModel):
    new_items: int = Field(ge=0)
    changed_items: int = Field(ge=0)
    missing_items: int = Field(ge=0)


class ResumeImportDraftResponse(_ResumeImportDomainModel):
...
class ResumeImportApplicationRequest(_ResumeImportDomainModel):
    draft_version: int = Field(ge=1)


class ResumeImportApplicationResponse(_ResumeImportDomainModel):
