...
from riva.services.training.catalog import (
...
class CompetencyIngestionService:
    ...
    async def ingest_practice_review(
        ...
        self._string_items(review.main_issues, "main_issues")
    ...
    async def ingest_interview_review(
        ...
        self._payload_string_items(payload, "frequentIssues")
        self._payload_string_items(payload, "riskPoints")
        if strengths:
            evidence.append(
                await self._add(
                    user_id=user_id,
