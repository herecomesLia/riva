...
from riva.services.job_description_analyses import new_job_description_analysis
...
class JobDescriptionImportDraftService:
    ...
    async def create_draft(
        ...
        *,
        ...
        if self.llm_provider is None or not self.llm_model:
            raise APIError(
                status.HTTP_503_SERVICE_UNAVAILABLE, IMPORT_DRAFT_PARSING_UNAVAILABLE
            )
        try:
    ...
    async def apply_draft(
        ...
        try:
            ...
            if (
                ...
                raise APIError(status.HTTP_409_CONFLICT, IMPORT_DRAFT_NOT_READY)
            try:
                output = JobDescriptionParsingOutput.model_validate(draft.parsed_result)
            except ValidationError:
                raise APIError(
                    status.HTTP_409_CONFLICT, IMPORT_DRAFT_NOT_READY
                ) from None
    ...
    async def _draft(
        ...
        if draft is None:
            raise APIError(status.HTTP_404_NOT_FOUND, IMPORT_DRAFT_NOT_FOUND)
        return draft
