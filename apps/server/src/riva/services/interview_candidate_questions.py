...
class InterviewCandidateQuestionService:
    ...
    async def submit_question(
        ...
        *,
        ...
        try:
            ...
            order = (
                int(
                    await self.session.scalar(
                        ...
                        .where(InterviewCandidateQuestion.session_id == session_id)
            ...
            input_snapshot = await self._build_input(interview_session, question)
            ...
            self.session.add(
                InterviewCandidateQuestionExchange(
                    ...
                    interviewer_answer=validated.interviewer_answer,
    ...
    async def _run(
        ...
        return (
            await InterviewCandidateQuestionAgent(
                self.llm_provider,
                self.llm_model,
            ).run(input_snapshot)
        ).output
        ...
        questions = list(
            (
                await self.session.scalars(
                    ...
                    .where(InterviewQuestion.session_id == interview_session.id)
    ...
    return InterviewCandidateCompletedQuestionSnapshot(
        ...
        follow_ups=[
            ...
            for follow_up in sorted(
                question.follow_up_questions, key=lambda item: item.order
            )
        ],
    )
