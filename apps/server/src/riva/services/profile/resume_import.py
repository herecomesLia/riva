"""Stateless resume text extraction used by ProfileService."""
...
@contextmanager
def _translate_extraction_errors(code: str) -> Iterator[None]:
    try:
        yield
        ...
        raise ResumeExtractionError(code) from None
...
class DefaultResumeTextExtractor:
    ...
    def _extract_pdf(self, data: bytes, *, max_characters: int) -> str:
        with _translate_extraction_errors(RESUME_PDF_INVALID):
            reader = PdfReader(
                BytesIO(data),
                strict=False,
        ...
        with _translate_extraction_errors(RESUME_PDF_EXTRACTION_FAILED):
    ...
    def _extract_docx(self, data: bytes, *, max_characters: int) -> str:
        with (
            _translate_extraction_errors(RESUME_DOCX_INVALID),
            zipfile.ZipFile(BytesIO(data), mode="r") as archive,
        ):
            ...
            for member_number, name in enumerate(xml_names):
                ...
                with _translate_extraction_errors(RESUME_DOCX_EXTRACTION_FAILED):
    ...
    def _read_xml_member(
        ...
        with (
            _translate_extraction_errors(RESUME_DOCX_INVALID),
            archive.open(info, mode="r") as member,
        ):
            while True:
                chunk = member.read(
...
def _parse_xml(data: bytes) -> ElementTree.Element:
    ...
    with _translate_extraction_errors(RESUME_DOCX_INVALID):
