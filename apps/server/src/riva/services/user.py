...
class UserService:
    def __init__(self, session: AsyncSession, config: AuthSessionConfig) -> None:
    ...
    async def register(self, username: str, password: str) -> AuthResult:
        ...
        try:
            ...
            raise UsernameTakenError("username_taken") from exc
    ...
    async def current_session(self, token: str) -> CurrentSession:
        ...
        refreshed = False
        refresh_after = timedelta(seconds=self.config.session_refresh_interval_seconds)
    ...
    def _new_session(self, user: User, now: datetime) -> tuple[str, AuthSession]:
        ...
        return token, AuthSession(
            user=user,
            token_digest=digest_session_token(token, self.config.session_digest_key),
            created_at=now,
            last_seen_at=now,
    ...
    def _expires_at(self, now: datetime) -> datetime:
        return now + timedelta(seconds=self.config.session_idle_timeout_seconds)
...
def _normalize_registration_username(username: str) -> str:
    try:
        ...
        raise InvalidDataError("invalid_username") from exc
...
def _validate_registration_password(password: str) -> None:
    try:
        ...
        raise InvalidDataError("invalid_password") from exc


def _invalid_credentials() -> ServiceError:
    return InvalidCredentialsError("invalid_credentials")
