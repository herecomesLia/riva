...
class TargetRoleService:
    ...
    async def update_job_description_analysis_module(
        ...
        try:
            ...
            if role.job_description_version != payload.job_description_version:
                raise APIError(
                    status.HTTP_409_CONFLICT, "job_description_version_conflict"
                )
            ...
            if (
                ...
                raise APIError(
                    status.HTTP_409_CONFLICT, "job_description_analysis_not_ready"
                )
    ...
    async def _locked_role(self, user_id: UUID, role_id: UUID) -> TargetRole:
        ...
        if role is None:
            raise APIError(status.HTTP_404_NOT_FOUND, "target_role_not_found")
        return role
    ...
    def _require_version(role: TargetRole, requested_version: int) -> None:
        if role.version != requested_version:
            raise APIError(status.HTTP_409_CONFLICT, "target_role_version_conflict")

    @staticmethod
    def _state_conflict() -> APIError:
        return APIError(status.HTTP_409_CONFLICT, "target_role_state_conflict")
    ...
    def _require_saved_job_description(role: TargetRole) -> None:
        if (
            ...
            raise APIError(status.HTTP_409_CONFLICT, "job_description_missing")

    def _require_llm(self, error_code: str) -> None:
        if self.llm_provider is None or not self.llm_model:
            raise APIError(status.HTTP_503_SERVICE_UNAVAILABLE, error_code)
...
__all__ = ["TargetRoleService", "build_matching_analysis_result_response"]
