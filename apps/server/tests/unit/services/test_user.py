...
def create_service(session: FakeSession) -> UserService:
    return UserService(  # type: ignore[arg-type]
        session,
        AuthSessionConfig(
            session_digest_key="test-session-key",
...
def test_empty_profile_update_does_not_write() -> None:
    session = FakeSession()
    service = create_service(session)
    user = create_user()
