...
from riva.integrations.storage.local import (
    RESUME_FILE_EMPTY,
...
from riva.services.resumes.documents import (
...
def assert_service_error(
    error: pytest.ExceptionInfo[ServiceError], status_code: int, code: str
) -> None:
    expected_type = {
        404: ResourceMissingError,
        413: DataTooLargeError,
        422: InvalidDataError,
        503: ExternalDependencyError,
    }[status_code]
    assert isinstance(error.value, expected_type)
...
def test_storage_errors_do_not_create_documents(
    ...
    assert_service_error(error, status_code, code)
...
def test_read_failure_and_unexpected_extractor_failure_compensate_storage() -> None:
    ...
    with pytest.raises(ServiceError) as read_error:
        asyncio.run(
    ...
    assert_service_error(read_error, 503, "resume_storage_unavailable")
    ...
    with pytest.raises(ServiceError) as extract_error:
        asyncio.run(
    ...
    assert_service_error(extract_error, 503, "resume_upload_unavailable")
...
def test_database_failure_rolls_back_and_compensates_storage(failure: str) -> None:
    ...
    assert_service_error(error, 503, "resume_upload_unavailable")
...
def test_list_get_and_load_are_user_scoped_and_explicitly_mapped() -> None:
    ...
    assert_service_error(missing_error, 404, "resume_document_not_found")
