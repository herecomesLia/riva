...
class FakeCareerProfileService:
    def __init__(
        ...
        *,
        replace_error: ServiceError | None = None,
    ) -> None:
...
def create_profile_client(
    ...
    app.dependency_overrides[require_career_profile_service] = lambda: profile_service
...
def test_put_profile_returns_version_conflict(app) -> None:
    service = FakeCareerProfileService(
        response_profile(),
        replace_error=DomainConflictError("profile_version_conflict"),
    )
    client, _user = create_profile_client(app, service)
